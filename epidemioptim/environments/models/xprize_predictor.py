...
DATA_PATH = get_repo_path() + "/data/model_data/"
DATA_FILE_PATH = DATA_PATH + "OxCGRT_latest.csv"
ADDITIONAL_CONTEXT_FILE = DATA_PATH + "Additional_Context_Data_Global.csv"
ADDITIONAL_US_STATES_CONTEXT = DATA_PATH + "US_states_populations.csv"
ADDITIONAL_UK_CONTEXT = DATA_PATH + "uk_populations.csv"
ADDITIONAL_BRAZIL_CONTEXT = DATA_PATH + "brazil_populations.csv"
IPS_FILE_PATH = DATA_PATH + "robojudge_test_scenario.csv"
# Fixed weights for the standard predictor.
MODEL_WEIGHTS_FILE = DATA_PATH + "trained_model_weights.h5"

# Date to make predictions
START_DATE_STR = "2020-08-01"
END_DATE_STR = "2020-08-05"
...
CONTEXT_COLUMNS = ["CountryName", "RegionName", "GeoID", "Date", "ConfirmedCases", "ConfirmedDeaths", "Population"]
NB_LOOKBACK_DAYS = 21
NB_TEST_DAYS = 14
WINDOW_SIZE = 7
...
class XPrizePredictor(BaseModel):
    """
    ...
    def __init__(
        ...
        path_to_ips_file=IPS_FILE_PATH,
        path_to_model_weights=MODEL_WEIGHTS_FILE,
        ...
        if path_to_model_weights:
            ...
            self.predictor, _ = self._construct_model(
                nb_context=nb_context, nb_action=nb_action, lstm_size=LSTM_SIZE, nb_lookback_days=NB_LOOKBACK_DAYS
            )
            self.predictor.load_weights(path_to_model_weights)

            # Make sure data is available to make predictions
            assert os.path.exists(DATA_FILE_PATH), f"Data file not found at {DATA_FILE_PATH}"
        ...
        # Prepare the output
        forecast = {"CountryName": [], "RegionName": [], "Date": [], "PredictedDailyNewCases": []}

        # Fix for past predictions
        self.geos = npis_df.GeoID.unique()
        self.df = self.df[self.df.Date < start_date_str]
        self.country_samples = self._create_country_samples(self.df, self.geos, False)
        # {
        #   'China': {
        #       "initial_context_input": (21 x 1),
        #       "action_input": (21 x 12)
        #       # meaning 1 row is 21 x 12, ie 21 days worth of NPI data
        #   },
        #   'United States' : {
        ...
        super().__init__(
            internal_states_labels=NPI_COLUMNS,
    ...
    def _sample_initial_state(self):
        self.initial_state = {
            g: {
                "context_input": value["X_test_context"][-1],
                "action_input": value["X_test_action"][-1],
            }
            for g, value in self.country_samples.items()
        }
    ...
    # Run n steps for selected geo_id
    def run_n_steps(self, geo, current_state=None, n=1):
        if current_state is None:
            current_state = self.initial_state

        pred_output = np.zeros(n)
        context_input = np.expand_dims(np.copy(current_state[geo]["context_input"]), axis=0)
        action_input = np.expand_dims(np.copy(current_state[geo]["action_input"]), axis=0)
        for d in range(n):
            # context_input: 1x21x12, action_input: 1x21x1
            pred = self.predictor.predict([context_input, action_input])
            pred_output[d] = pred
            context_input[:, :-1] = context_input[:, 1:]
            context_input[:, -1] = pred
        self.current_state[geo]["context_input"] = context_input
        self.current_state[geo]["action_input"] = action_input
        return self.current_state

    def _get_new_cases_preds(self, c_df, g, npis_df, country_samples):
        cdf = c_df[c_df.ConfirmedCases.notnull()]
        initial_context_input = country_samples[g]["X_test_context"][-1]  # shape 21x12 npis for the past 21 days
        initial_action_input = country_samples[g]["X_test_action"][-1]  # shape 21x1 cases_ratio for the past 21 days

        # Predictions with passed npis
        cnpis_df = npis_df[npis_df.GeoID == g]
        npis_sequence = np.array(cnpis_df[NPI_COLUMNS])  # The next 5 days npi sequence
        # Get the predictions with the passed NPIs
        preds = self._roll_out_predictions(self.predictor, initial_context_input, initial_action_input, npis_sequence)
    ...
    def load_original_data(data_url):
        ...
        latest_df["GeoID"] = np.where(
            latest_df["RegionName"].isnull(),
    ...
    @staticmethod
    def _convert_ratio_to_new_cases(ratio, window_size, prev_new_cases_list, prev_pct_infected):
        return (ratio * (1 - prev_pct_infected) - 1) * (
            window_size * np.mean(prev_new_cases_list[-window_size:])
        ) + prev_new_cases_list[-window_size]

    def _convert_ratios_to_total_cases(self, ratios, window_size, prev_new_cases, initial_total_cases, pop_size):
        new_new_cases = []
        prev_new_cases_list = list(prev_new_cases)
        curr_total_cases = initial_total_cases
        for ratio in ratios:
            new_cases = self._convert_ratio_to_new_cases(
                ratio, window_size, prev_new_cases_list, curr_total_cases / pop_size
            )
    ...
    def train(self):
        ...
        for t in range(NUM_TRIALS):
            ...
            model, training_model = self._construct_model(
        ...
        country_casess = []
        for model in models:
            country_indep, country_preds, country_cases = self._lstm_get_test_rollouts(
                model, self.df, geos, country_samples
            )
    ...
    def _construct_model(self, nb_context, nb_action, lstm_size=32, nb_lookback_days=21):
        ...
        context_input = Input(shape=(nb_lookback_days, nb_context), name="context_input")
        x = LSTM(lstm_size, name="context_lstm")(context_input)
        context_output = Dense(units=1, activation="softplus", name="context_dense")(x)

        # Create action encoder
        # Every aspect is monotonic and nonnegative except final bias
        action_input = Input(shape=(nb_lookback_days, nb_action), name="action_input")
        x = LSTM(
            units=lstm_size,
        ...
        action_output = Dense(units=1, activation="sigmoid", kernel_constraint=Positive(), name="action_dense")(x)

        # Create prediction model
        model_output = Lambda(_combine_r_and_d, name="prediction")([context_output, action_output])
    ...
    def _lstm_get_test_rollouts(self, model, df, top_geos, country_samples):
        ...
        for g in top_geos:
            ...
            preds = self._lstm_roll_out_predictions(
                model, initial_context_input, initial_action_input, future_action_sequence
            )
...
if __name__ == "__main__":
    # Get model
    model = XPrizePredictor()

    # Run simulation
    simulation_horizon = 364
    model_states = model.run_n_steps(geo="United States", n=5)
    print(model_states['United States']['action_input'])
